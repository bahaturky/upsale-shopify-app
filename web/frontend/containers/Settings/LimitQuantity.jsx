...
const LimitQuantityOption = ({ t, index, option, setProductPickerProps, setCollectionPickerProps }) => {
  const [{ value }, , { setValue }] = useField("limit");
  const { options = [] } = value;
  const { quantity, collections = [], products = [] } = option;

  const onRemoveOption = useCallback(
    () => setValue({ ...value, options: options.filter((x) => x !== option) }),
  ...
  const onQuantityReset = useCallback(() => {
    setValue({
      ...value,
      options: options.map((o) => (o !== option ? o : { ...option, quantity: EMPTY_VALUE })),
    });
  }, [value, options, option, setValue]);
  ...
  return (
    <Fragment key={index}>
      <Card.Section
        title={t("limit-quantity-option-title").replace("{number}", index + 1)}
        actions={[
          {
      ...
      <ResourceList
        emptyState={
          <p style={{ opacity: 0.5, paddingLeft: "2rem", paddingRight: "2rem" }}>
    ...
    <>
      ...
      >
        ...
        {options.map((option, index) => (
          <LimitQuantityOption key={index} {...{ t, index, option, setProductPickerProps, setCollectionPickerProps }} />
        ))}
      </Card>
              ...
              options: options.map((o) =>
                o !== productPickerProps.option
                  ...
                  : {
                      ...
                      products: selection.map(({ id, handle, title, descriptionHtml, images }) => ({
                        id,
                        handle,
                        title,
                        descriptionHtml,
      ...
      {collectionPickerProps.open ? (
        <ResourcePicker
          ...
          onSelection={({ selection }) => {
              ...
              options: options.map((o) =>
                o !== collectionPickerProps.option
                  ...
                  : {
                      ...
                      collections: selection.map(({ id, handle, title, descriptionHtml, image }) => ({
                        id,
                        handle,
                        title,
                        descriptionHtml,
            ...
            setCollectionPickerProps((state) => ({ ...state, open: false }));
          }}
          onCancel={() => setCollectionPickerProps((state) => ({ ...state, open: false }))}
